...
class ChatHistoryService:
    ...
    def list_conversations(
        self, uow: UnitOfWork, owner_id: str,
    ) -> list[ChatConversation]:
        return uow.chats.list_conversations(owner_id)

    def create_conversation(
        self, uow: UnitOfWork, owner_id: str, title: str,
    ) -> ChatConversation:
        conversation = uow.chats.create_conversation(owner_id, title.strip()[:100])
        uow.commit()
        return conversation

    def get_conversation(
        self, uow: UnitOfWork, owner_id: str, conversation_id: str,
    ) -> tuple[ChatConversation, list[ChatMessage]]:
        conversation = uow.chats.get_conversation(owner_id, conversation_id)
    ...
    def append_message(
        ...
        *,
        ...
        if uow.chats.get_conversation(owner_id, conversation_id) is None:
            raise ConversationNotFound("找不到這筆診斷紀錄")
