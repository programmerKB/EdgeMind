...
DB_NAME=agent-postgres
BACKEND_NAME=agent-fastapi
FRONTEND_NAME=agent-frontend
NETWORK=edgemind-dev-net
# Keep the legacy volume name so existing development data remains available.
DB_VOLUME=my_agent_project_postgres_dev_data
...
[[ $# -eq 0 ]] || die "此腳本不接受參數"
[[ -f $ENV_FILE ]] || die '找不到 .env.dev 或 .env；請先複製 .env.example 並填入設定'
...
printf '建置開發映像...\n'
docker build --target development -t edgemind-backend:dev "$ROOT_DIR/backend"
docker build --target development -t edgemind-frontend:dev "$ROOT_DIR/frontend"
...
docker run -d --name "$BACKEND_NAME" --network "$NETWORK" --network-alias backend \
  ...
  --mount "type=bind,src=$ROOT_DIR/backend,dst=/app" \
  edgemind-backend:dev >/dev/null
wait_healthy "$BACKEND_NAME"

docker run -d --name "$FRONTEND_NAME" --network "$NETWORK" \
  --restart unless-stopped --publish "$APP_PORT:5173" \
  --env VITE_PROXY_TARGET=http://backend:8000 \
  --mount "type=bind,src=$ROOT_DIR/frontend,dst=/app" \
  --mount type=volume,dst=/app/node_modules \
  edgemind-frontend:dev >/dev/null
wait_healthy "$FRONTEND_NAME"

printf '開發環境已就緒：Web http://localhost:%s；API http://127.0.0.1:%s/docs\n' "$APP_PORT" "$BACKEND_PORT"
